...
class AircraftActorSpec extends ScalaTestWithActorTestKit with AnyWordSpecLike {
  ...
  "AircraftActor" must {
    ...
    "reply with latest Telemetry" in {
      ...
      val at1 = TrackTelemetry(0L,a1.addr,loc=Location(1,1,Altitude(100,Units.METERS)),hSpeed=Speed(60,Units.KNOTS),vRate=VRate(5,Units.FPM),heading=0.0)
      val at2 = TrackTelemetry(0L,a1.addr,loc=Location(2,2,Altitude(200,Units.METERS)),hSpeed=Speed(70,Units.KNOTS),vRate=VRate(8,Units.FPM),heading=0.0)
