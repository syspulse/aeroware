...
class ParqFileRotator(file:String,ts0:Long = Long.MaxValue,flushes:Int = 1000) extends AutoCloseable {
  ...
  var numWrites = 0
  var nextTs = 0L
  var pw:Option[ParquetWriter[MinedData]] = None

  rotate()

  def isRotate() = System.currentTimeMillis() >= nextTs

  def rotate():Option[ParquetWriter[MinedData]] = {
    close()
    ...
    try {
      ...
      pw = Some(ParquetWriter.of[MinedData].build(Path(f)))
      pw
...
class MinedStoreParq(dir:String = "./lake/{addr}/data-{yyyy}-{MM}-{dd}_{HH}:{mm}/data-{id}.parq",flushes:Int = 1000)(implicit config:Config) extends MinedStore {
